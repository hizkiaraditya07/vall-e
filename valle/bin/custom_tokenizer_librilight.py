...
def main():
    ...
    if args.audio_extractor == "Encodec": 
        ...
        # extractor = AudioTokenExtractor(AudioTokenConfig(frame_shift=320/16000))
        # print(extractor.tokenizer.sample_rate)
    else:
    ...
    with get_executor() as ex:
        for partition, m in manifests.items():
            ...
            for c in tqdm(cut_set):
                ...
                elif args.prefix == "librilight":
                    phonemes = tokenize_text(
                        text_tokenizer, text=c.supervisions[0].text
                    )
                else:
                    # prefix is prosa
                    phonemes = [char if not(char.isspace()) else '_' for char in c.supervisions[0].text]
